  ...
  if (pInput === "Rock") {
    if (cInput === "Scissors") {
      ...
      document.getElementById(
        "case"
      ).textContent = `${currentMatch} = Computer Wins`;
      cScore++;
      round++;
    }
...
function countdown() {
  /* inside of the checkwinner */
  seconds = 3;
  document.getElementById("countdown").textContent =
    seconds + " sec to start !";
  choices.forEach((buttons) => {
    /*disables choice buttons when countdown starts */
  ...
  var count = setInterval(function () {
    ...
    if (seconds < 0) {
      ...
      choices.forEach((buttons) => {
        /*activates choice buttons when countdown ends */
        const idOfButtons = buttons.id;
